...
`stash connect <remote>` — connect to existing remote; requires `--name`; pulls into new local stash. Syncs Automerge from remote, then renders files to disk via reconciler flush.  
`stash list` — list stashes with name, description, path, remote. During load, stale registrations (missing local stash path) are removed automatically and user is notified.  
...
`stash sync [name]` — scan disk, sync Automerge, then render updated files to disk via reconciler flush; optional single stash by name.  
